...
const Header = () => {
  ...
  return (
    ...
    transition={{ default: {duration: 0.5}, opacity: {delay:0.8, duration:1}}}
      className='cursor-pointer sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full'   onClick={onClickHandler}>Generate Images <img src={assets.star_group} alt="" className='h-6' 
    ...
    transition={{delay:1, duration:1}}
     ...
     className='flex flex-wrap justify-center mt-16 gap-3'>
        { Array(6).fill('').map((items, index)=>(
            ...
            src={
                index % 2 == 0 ? assets.sample_img_2 :  
                assets.sample_img_1} alt="" key={index} className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' width={70} />
        ))}
     </motion.div>
