...
const BuyCredit = () => {
  ...
  const {user,backendUrl, token, getCreditData, setShowLogin} = useContext(AppContext);
       ...
       <div  className='flex flex-wrap justify-center gap-6 text-left'>
        {
          plans.map((plan, index)=>(
            <div  className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500' key={index}>
              ...
              <button onClick={()=> PaymentRazorPay(plan.id)}  className='bg-gray-800 mt-8 text-white rounded-md  text-sm py-2.5 min-w-52 w-full cursor-pointer'>{ user ? 'Purchase' : 'Get Started'}</button>
